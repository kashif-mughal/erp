...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        ?>
        <div class="alert alert-danger alert-dismissable" id="grandTotalError" style="display: none;">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            Grand total could not be more then credit limit.
        </div>
        <div class="row">
            <div class="col-sm-12">
                <div class="column">
                            ...
                            <table class="table table-bordered table-hover" id="normalinvoice">
                                ...
                                <tbody id="addinvoiceItem">
                                    <tr>
                                        ...
                                        <!-- Discount -->
                                        <td>
                                            <input type="text" name="discount[]" onkeyup="quantity_calculate(1);" onkeydown="change_discount(event);" onchange="quantity_calculate(1);" id="discount_1" class="form-control text-right" min="0" placeholder="0.00"/>
...
<script type="text/javascript">
    ...
    function clearEmptyRows(){
        var allTrs = $('#addinvoiceItem tr');
        var filteredTrs = [];
        for(var i = 0; i < allTrs.length;i++){
        var temp = $(allTrs[i]).find('.productSelection')[0].value;
            if(temp){
                filteredTrs.push(allTrs[i]);
            }
        }
        $('#addinvoiceItem tr').remove();
        for(var i = 0; i< filteredTrs.length; i++){
            $('#addinvoiceItem').append(filteredTrs[i]);
        }
    }
    $('#sale_order_number').on("blur", function(){
        $.ajax({
            url : 'Cinvoice/sale_order_remaining_items',
            type : 'GET',
            data : {
                'saleOrderId' : parseInt($('#sale_order_number').val())
            },
            dataType:'json',
            success : function(data) {debugger;
                $('.row-from-sale-order').remove()
                for (var i = 0; i < data.length; i++) {
                    addInputField('addinvoiceItem', data[i]);
                }
                if(data && data.length > 0)
                {
                    $('#customer_id').val(data[0].so_customer_id).trigger('change');
                }
                clearEmptyRows();
