...
//print_r($query1);exit;
    ...
    public function monthly_sales_report_mk($from_date = null, $to_date = null) {
        $queryText = "
            SELECT 
                DATE_FORMAT(date,'%M-%Y') as date,
                EXTRACT(MONTH FROM STR_TO_DATE(date,'%Y-%m-%d')) as month,
                TRUNCATE(IFNULL(sum(total_amount), 0), 2) as gross_sales,
                TRUNCATE(IFNULL(sum(total_ret_amount), 0), 2) as sale_return,
                TRUNCATE(sum(total_amount) - IFNULL(sum(total_ret_amount), 0), 2) as net_sale
    ...
    public function GetOpeningSales($openingMonth){
        $netSalesQuery = "
            SELECT 
                TRUNCATE(sum(total_amount) - IFNULL(sum(total_ret_amount), 0), 2) as net_sale
            ...
            LEFT JOIN
                ...
                on invoice.invoice_id = product_return.invoice_id
            WHERE 
                STR_TO_DATE(date,'%Y-%m-%d') < STR_TO_DATE('$openingMonth','%Y-%m-%d')";

        $netSalesResult = $this->db->query($netSalesQuery)->result_array();


        $recieveQuery = "
            SELECT 
                IFNULL(sum(pay_amount), 0) as total_amount
            FROM 
                transection
            WHERE 
                STR_TO_DATE(date_of_transection,'%Y-%m-%d') < STR_TO_DATE('$openingMonth','%Y-%m-%d')";
        //print_r($queryText);die;
        $recieveQueryResult = $this->db->query($recieveQuery)->result_array();
        $openingAmount = $netSalesResult[0]["net_sale"] - $recieveQueryResult[0]["total_amount"];
